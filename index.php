...
<html lang="en">
  <head>
    ...
    <title>Simulador de Matriculas</title>
  </head>
  <body>
    <div class="container"><br>
      <a class="btn btn-primary float-right" href="admin/login.php">Panel Administrativo</a>
          <h1 class="text-center"> Simulación de Matrícula </h1><br>
          ...
          <div class="row">
            ...
            <table class="text-center infotable">
              ...
              <tr>
                <td>
                  <p><label for="id">Número Matricula</label></p>
                </td>
                <td>
                  <input class="form-control" type="text" pattern="[0-9]{6}" id="id" placeholder="6 dígitos..." name="id">
                </td>
              </tr>
              <tr>
        ...
        <?php if (isset($_POST['showinfo'])) {
          ...
          $id = $_POST['id'];
          if (!empty($choose && $id)) {
            $query = mysqli_query($db_con,"SELECT * FROM `student_info` WHERE `id`='$id' AND `class`='$choose'");
            if (!empty($row=mysqli_fetch_array($query))) {
              if ($row['id']==$id && $choose==$row['class']) {
                $stid= $row['id'];
                $stname= $row['name'];
                $stclass= $row['class'];
        ...
        <div class="row">
          <div class="col-sm-6 offset-sm-3">
            <table class="table table-bordered">
              <tr>
                <td rowspan="4"><h3>Información de Estudiante</h3><img class="img-thumbnail" src="admin/images/<?= isset($photo)?$photo:'';?>" width="250px"></td>
                <td>Número de Matrícula</td>
                <td><?= isset($stid)?$stid:'';?></td>
              </tr>
                <td>Nombre</td>
